...
final class RemoteLoaderTests: XCTestCase {
    ...
    func test_init_doesNotRequestDataFromURL() {
        let (_, client) = makeSUT(mapper: { _, _ in "any" })
    ...
    func test_load_doesNotDeliverResultAfterSUTInstanceHasBeenDeallocated() {
        ...
        var sut: RemoteLoader<String>? = RemoteLoader<String>(url: anyURL(), client: client, mapper: { _, _ in "any" })
    ...
    private func expect(_ sut: RemoteLoader<String>, toCompleteWith expectedResult: RemoteLoader<String>.Result, when action: () -> Void, file: StaticString = #filePath, line: UInt = #line) {
        ...
        let expection = expectation(description: "Wait for load completion")
